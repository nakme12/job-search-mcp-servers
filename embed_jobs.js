...
const outHtml = path.resolve('D:/adi/mcp/jobs_embedded.html');

const raw = fs.readFileSync(jsonPath, 'utf-8');
const jobsArray = raw.trim();
...
<body>
  ...
  <script>
    ...
    function render(list){tbody.innerHTML='';list.forEach((j,i)=>{const tr=document.createElement('tr');tr.innerHTML=`<td>${i+1}</td><td>${j.title||''}</td><td>${j.company||''}</td><td>${j.location||''}</td><td>${j.remote===true?'Yes':j.remote===false?'No':''}</td><td>${j.date||''}</td><td>${j.src||''}</td><td><a href="${j.url}" target="_blank" rel="noopener">View</a></td>`;tbody.appendChild(tr);});}
    function applyFilters(all){const term=document.getElementById('searchBox').value.toLowerCase();const loc=document.getElementById('locationFilter').value;return all.filter(j=>{const matchSearch=(j.title&&j.title.toLowerCase().includes(term))||(j.company&&j.company.toLowerCase().includes(term))||(j.location&&j.location.toLowerCase().includes(term));const matchLoc=loc==='All'||(j.location&&j.location.includes(loc));return matchSearch&&matchLoc;});}
    const allJobs = jobs;
    populateLocationSelect(allJobs);
    render(allJobs);
    document.getElementById('searchBox').addEventListener('input',()=>render(applyFilters(allJobs)));
    document.getElementById('locationFilter').addEventListener('change',()=>render(applyFilters(allJobs)));
  </script>
</body>
</html>`;

fs.writeFileSync(outHtml, html, 'utf-8');
console.log('Embedded HTML written to', outHtml);
